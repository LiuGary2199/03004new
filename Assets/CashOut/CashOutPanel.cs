...
public class CashOutPanel : FirmUICharm
{
    ...
    private void Start()
    {
        CashOutManager.BisFeedback()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { HappyUIMust(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = TanGenuUSA.instance.BillPpm_Span.MoneyName;
        InfoText.text = TanGenuUSA.instance.BillPpm_Span.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = TanGenuUSA.instance.FirmCry;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = TanGenuUSA.instance.FirmCry;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.BisFeedback().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.BisFeedback().Account;
        Confirm_AccountText.text = CashOutManager.BisFeedback().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.BisFeedback().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = TanGenuUSA.instance.FirmCry + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = AkinSpanThinker.BisLuck("CashOut_PolicyAgree");
    ...
    {
        if (CashOutManager.BisFeedback().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.BisFeedback().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            CheerThinker.BisFeedback().FirnCheer("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            CheerThinker.BisFeedback().FirnCheer(AccountErrorText.text);
            return;
        }

        CashOutManager.BisFeedback().Account = AccountInput.text;
        AkinSpanThinker.GapAttest("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            CheerThinker.BisFeedback().FirnCheer("Please agree to the policy");
            return;
        }
        AkinSpanThinker.GapLuck("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.BisFeedback().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.BisFeedback().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.BisFeedback().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(TanGenuUSA.instance.BillPpm_Span.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.BisFeedback().Data.Cash;
    ...
    {
        if (CashOutManager.BisFeedback().Data.TaskData != null && CashOutManager.BisFeedback().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.BisFeedback().Data.TaskData.Name == "Cash" && CashOutManager.BisFeedback().Data.Cash < CashOutManager.BisFeedback().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.BisFeedback().Data.TaskData.Description, CashOutManager.BisFeedback().Data.Cash, CashOutManager.BisFeedback().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.BisFeedback().Data.TaskData.Name != "Cash" && CashOutManager.BisFeedback().Data.TaskData.NowValue < CashOutManager.BisFeedback().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.BisFeedback().Data.TaskData.Description, CashOutManager.BisFeedback().Data.TaskData.NowValue, CashOutManager.BisFeedback().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.BisFeedback().Data.Record == null || CashOutManager.BisFeedback().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.BisFeedback().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.BisFeedback().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            CheerThinker.BisFeedback().FirnCheer("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        SelfSheerRibbon.BisFeedback().CastSheer("1301", LeftTimeText.text, CashOutManager.BisFeedback().Money.ToString());
    }
}
