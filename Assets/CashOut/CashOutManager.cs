...
public class CashOutManager : GistThickness<CashOutManager>
{
    [Header("登录平台")]
    ...
    private void Start()
    {
        Account = AkinSpanThinker.BisAttest("CashOut_Account");
        Money = AkinSpanThinker.BisOught("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {TanGenuUSA.instance.BillPpm_Span.MoneyName} have been converted,Please check your rewards!";
            IrreversibleThinker.Feedback.StaveIrreversible();
            IrreversibleThinker.Feedback.ScavengeIrreversible(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                IrreversibleThinker.Feedback.ScavengeIrreversible(title, info, (int)Seconds + (i * 10800));
        }
    ...
    void TimeCount() //计时
    {
        if (Data != null)
        {
            ...
            //任务
            if (TanGenuUSA.instance.BillPpm_Span != null && TanGenuUSA.instance.BillPpm_Span.TaskList.Count > 0)
            {
                //记录第一次登录日期utc
                ...
                if (Day >= TanGenuUSA.instance.BillPpm_Span.TaskList.Count) //一天一任务 天数超出任务数量显示默认任务
                    Data.TaskData = TanGenuUSA.instance.BillPpm_Span.TaskList.FirstOrDefault(t => t.IsDefault);
                else
                    Data.TaskData = TanGenuUSA.instance.BillPpm_Span.TaskList[Day];
    ...
    {
        Money += Value;
        AkinSpanThinker.GapOught("CashOut_Money", Money);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        SelfSheerRibbon.BisFeedback().CastSheer("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", AkinSpanThinker.BisAttest("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        {
            Platform = "Android";
            DeviceAdId = AkinSpanThinker.BisAttest("gaid");
            if (Application.platform == RuntimePlatform.Android)
            {
        ...
        else
        {
            Platform = "iOS";
            Manufacturer = "Apple";
            DeviceAdId = AkinSpanThinker.BisAttest("idfv");
        }
        ...
        bool hasuuidAndAppid = AkinSpanThinker.BisLuck("UuidAndAPPid");
        ...
        TanToilThinker.BisFeedback().EmitSelfGain(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        bool isNewPlayer = !PlayerPrefs.HasKey(CChisel.Ox_ItRatVictim + "Bool") || AkinSpanThinker.BisLuck(CChisel.Ox_ItRatVictim);
                        if (isNewPlayer)
                            AkinSpanThinker.GapLuck("UuidAndAPPid", true);
#endif

                        //刷新token 获取提现规则
                        AkinSpanThinker.GapAttest("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(AkinSpanThinker.BisAttest("CashOut_ConvertTime"));
                        ...
                        AkinSpanThinker.GapAttest("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    else
                    {
                        ...
                        CheerThinker.BisFeedback().FirnCheer("Login fail :" + response.msg);
                    }
                }
                catch (Exception e)
                {
            ...
            fail: () =>
            {
                ...
                CheerThinker.BisFeedback().FirnCheer("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        TanToilThinker.BisFeedback().EmitBis(
            url: url,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = AkinSpanThinker.BisHeyday("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                SelfSheerRibbon.BisFeedback().CastSheer("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        AkinSpanThinker.GapHeyday("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            CheerThinker.BisFeedback().FirnCheer($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        TanToilThinker.BisFeedback().EmitSelfGain(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    CheerThinker.BisFeedback().FirnCheer("Withdraw fail :" + e.Message);
    ...
    {
        SelfSheerRibbon.BisFeedback().CastSheer("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = VagaryBiteThinker.Instance.BisVagaryBath();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    {
        if (string.IsNullOrEmpty(AkinSpanThinker.BisAttest("CashOut_Token")))
        {
    ...
    public void AddTaskValue(string Name, float Value) //增加任务值
    {
        if (Data.TaskData != null && Data.TaskData.Name == Name)
        {
            ...
            {
                SelfSheerRibbon.BisFeedback().CastSheer("1305", Name, NewValue.ToString(), Data.TaskData.IsDefault.ToString());
