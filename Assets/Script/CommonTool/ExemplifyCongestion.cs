...
public class ExemplifyCongestion : MonoBehaviour
{
    void Start()
    {
    ...
    public static void HolmOn(GameObject IconImage, float IconMoveYStart, float IconMoveYFinal,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void CudFirn(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void CudSafe(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void SteppeCarton(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void SteppeCarton(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        SteppeCarton(startNum, endNum, delay, text, "", finish);
    }
    public static void SteppeCarton(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + CartonLift.HeydayMyEke(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }

    public static void ExtolOn(float startNum, float endNum,int nowLevel,int fullCount , float delay, Image image,Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void SteppeSectorStilt(float startValue,float endValue,Image sliderImage, System.Action finish, float delay = 0)
    {
    ...
    /// <param name="finish"></param>
    public static void VaseFirn(GameObject HomeTopBarShow, float MoveOffsetShow, System.Action finish) 
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    /// <param name="MoveOffset"></param>
    /// <param name="finish"></param>
    public static void VaseSafe(GameObject HomeTopBarHide, float MoveOffsetHide, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void ChopGaze(GameObject GoldImage,Transform parent, int a ,Transform GoldStart,Transform GoldFinal,System.Action finish) 
    {
        if (a == 0)
        {
            finish();
    ...
    public static void ChopGazeDart(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            s.Append(GoldIcon.transform.DOMove(new Vector3(GoldIcon.transform.position.x + OffsetX, GoldIcon.transform.position.y + OffsetY, GoldIcon.transform.position.z), 0.15f).SetDelay(Delaytime).OnComplete(() =>
            {
                ...
                if (t % 5 == 0)
                {
                    GooseUSA.BisFeedback().LionRotate(GooseCity.UIMusic.Sound_GoldCoin);
                }
            }));
            //金币移动到最终位置
    ...
    /// <param name="i"></param>
    public static void SoulBillGaze(GameObject GameBankCashImage,Transform parent,Transform BankCashStart,Transform BankCashFinal,int i,System.Action finish)
    {
        ...
        if (i <= 2)
        {
            ...
            GameBankCash.transform.DOMoveY(BankCashFinal.position.y,0.7f).OnComplete(()=>{
                ...
                finish();
                //GooseUSA.GetInstance().PlayEffect(GooseCity.SceneMusic.Sound_GameBankGold);
            });
        }
        else if( i==3 )
        {
            ...
            GameBankCash.transform.DOMoveY(BankCashFinal.position.y, 0.7f).OnComplete(() => 
            {
                ...
                //GooseUSA.GetInstance().PlayEffect(GooseCity.SceneMusic.Sound_BankCashFull);
    ...
    /// <param name="finish"></param>
    public static void SoonKeyGaze(GameObject StarBox,float BoxMoveOffset,GameObject Fx_StarBoxOpen, System.Action RewardShow) 
    {
    ...
    /// <param name="finish"></param>
    public static void OverKeyFecundFirn(List<GameObject> CardList, int Count, System.Action finish)
    {
        ...
        float screenW = UIManager.BisFeedback().VerbUsable.GetComponent<CanvasScaler>().referenceResolution.x;
        int finishCount = Count;
        if (Count == 3) 
        {
            float Space1 = 80f;
            float Space2 = ((UIManager.BisFeedback().VerbUsable.GetComponent<CanvasScaler>().referenceResolution.x - Space1 * 2) - CardW * Count) / (Count - 1);
    ...
    /// <param name="finish"></param>
    public static void ExtolSunsetIArcIrony(GameObject UnLevelIcon, Sprite LevelIcon, Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void ShapeServeUrn(GameObject box,GameObject Fx_RewardBG,System.Action finish) 
    {
    ...
    /// <param name="i"></param>
    public static void Optimal(GameObject BankBtn,int i) 
    {
        float offset = -0.6f;
    ...
    public static void HolmRuin(GameObject IconImage)
    {
    ...
    public static void OVOGaze(GameObject GoldImage, Transform parent, int a, Transform GoldStart, Transform GoldFinal, System.Action NextShow, System.Action finish)
    {
        if (a == 0)
        {
            finish();
    ...
    /// <param name="Finish"></param>
    public static void UnforeseenExpose(GameObject obj, float addPosition, System.Action Finish)
    {
    ...
    public static void HolmSect(GameObject Tell)
    {
        Tell.transform.DOScale(0.52f, 0.08f).OnComplete(() => 
        {
            Tell.transform.DOScale(0.7f, 0.08f).OnComplete(()=> 
            {
                Tell.transform.DOScale(0.52f, 0.08f);
            }); 
        });
    }
    
    public static void RadiantUrn(Vector3 target, GameObject Tell, Vector3 Start, Transform parent,
        System.Action finish)
    {
        ...
        {
            Delaytime += 0.1f;
            GameObject GoldItem = Instantiate(Tell, parent);
    ...
    /// fever模式动画
    /// </summary>
    IEnumerable  CruelPistachio(List<GameObject> Lightlist) 
    {
    ...
    public static void RadiantFecundUrn(Vector3 target, GameObject Tell, Vector3 Start, Transform parent,
        System.Action finish)
    {
        GameObject GoldItem = Instantiate(Tell, parent);
