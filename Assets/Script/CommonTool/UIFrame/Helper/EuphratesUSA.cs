...
public class EuphratesUSA : MonoBehaviour
{
    /* 字段 */
    private static EuphratesUSA _Feedback;              //本脚本私有单例实例
    private Hashtable An= null;                        //容器键值对集合
    ...
    /// </summary>
    /// <returns></returns>
    public static EuphratesUSA BisFeedback()
    {
        if (_Feedback == null)
        {
            _Feedback = new GameObject("_ResourceMgr").AddComponent<EuphratesUSA>();
        }
        return _Feedback;
    }
    ...
    /// <param name="isCatch"></param>
    /// <returns></returns>
    public GameObject WantQuiet(string path, bool isCatch)
    {
