...
public class SnakeCanyon : MonoBehaviour
{
    /// <summary>
    /// 查找子节点对象
    /// 内部使用递归
    ...
    /// <returns></returns>
    public static Transform PickSodGuessLike(GameObject goParent,string childName)
    {
        //查找结果
        ...
        if (searchTrans == null)
        {
            ...
            {
                searchTrans = PickSodGuessLike(trans.gameObject, childName);
                if (searchTrans != null)
                {
                    return searchTrans;
                }
    ...
    /// <returns></returns>
    public static T BisSodSmellLikeWorksheetDoorway<T>(GameObject goParent,string childName) where T:Component
    {
        //查找特定子节点
        Transform searchTranformNode = null;
        searchTranformNode = PickSodGuessLike(goParent, childName);
    ...
    /// <param name="childName">子对象名称</param>
    /// <returns></returns>
    public static T BisSmellLikeWorksheet<T>(GameObject goParent,string childName) where T : Component
    {
        Transform searchTransform = null;
        searchTransform = PickSodGuessLike(goParent, childName);
    ...
    public static void BisSmellLikeMyAutumnLike(Transform parents,Transform child)
    {
