...
{
    public static void Ounce<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class BisIndigoSpan : GistThickness<BisIndigoSpan>
{
    // Start is called before the first frame update
    public bool SpanHearth;
    public bool ToRend;

    static public BisIndigoSpan instance;
    ...
    /// <returns></returns>
    public Vector3 RoeSepalBlaze(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject BisFalloutWidePiston(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float BlazeMySpill(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Spill180My360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int BisSnakeParagraph(Vector2 p1, Vector2 p2)
    {
        float angle = Spill180My360(BlazeMySpill(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 RoeTopsoilGlueLoan(Renderer Renderer)
    {
    ...
    public Vector2 RoeTopsoilGlueLoan(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 RoeLengthLoan(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 RoeLargeUntapped()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = RoeClientAuger();
        float screenH = RoeClientCloudy();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int RoeHardenAngry(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] BisMomentUnproven(int total, int count)
    {
    ...
    public Dictionary<string, object> AttestMyUnchanging(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string UnchangingMyAttest(Dictionary<string, object> Info)
    {
    ...
    public int HimDrawInto(string dateStart, DateTime dateEnd)
    {
    ...
    public int AlpDrawReal(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan DrawReal(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 BisYamOrLoss(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = BisYamOrLoss(canvas, text, strFragmentIndex);
            Vector3 lastPos = BisYamOrLoss(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = BisYamOrLoss(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 BisYamOrLoss(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float InscribeOxExam(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] From= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string PromoteDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string ObscureDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = From;
