...
public class ATaskPanel_B : FirmUICharm
{
    ...
    private void MoteKilnHunt()
    {
        ...
        for (int i = 0; i < WakeKarst.Count && i < WakePetiole.Length; i++)
        {
            if (WakeKarst[i] != null)
            {
                ...
                int progress = WideSpanThinker.BisFeedback().WedBallAtlantic(taskId);
                TaskStatus_B status = WideSpanThinker.BisFeedback().WedBallLimner(taskId);
