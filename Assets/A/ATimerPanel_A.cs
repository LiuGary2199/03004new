...
public class ATimerPanel_A : FirmUICharm
{
...
[UnityEngine.Serialization.FormerlySerializedAs("TimerItemList")]    public List<ATimerItem_A> VagueWeltHunt= new List<ATimerItem_A>();
    ...
    {
        base.Display();
        DigAdopt.onClick.AddListener(() => { HappyUIMust(GetType().Name); });
        // 检查并刷新每日计时器
    ...
    private void CrustalVagueHunt()
    {
        for (int i = 0; i < GourdKarst.Count; i++)
        {
            ...
            {
                int GourdIt= i;
                int duration = WideSpanThinker.BisFeedback().WedEssayStrength(GourdIt);
                ATimerStatus_A status = WideSpanThinker.BisFeedback().WedEssayLimner(GourdIt);
