...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].FirmCry != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADThinker>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADThinker>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADThinker>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].WideTusk != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADThinker>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADThinker>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADThinker>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<VagaryBiteThinker>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<VagaryBiteThinker>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<VagaryBiteThinker>();
            }
        }
        GameObject.Find("MainManager").GetComponent<VagaryBiteThinker>().UniqueID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<WarnAnThinker>())
        {
            GameObject.Find("MainManager").AddComponent<WarnAnThinker>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<WarnAnThinker>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<WarnAnThinker>().appid = info.PackageName;
#endif
        ...
        Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].WideTusk = info.GameCode;
        Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].FirmCry = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].FirmIdiomCry = info.BaseUrl + CChisel.IdiomCry;
        Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].FirmChiselCry = info.BaseUrl + CChisel.ChiselCry;
        Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].FirmTiltCry = info.BaseUrl + CChisel.TiltCry;
        Resources.FindObjectsOfTypeAll<TanGenuUSA>()[0].FirmVagaryCry = info.BaseUrl + CChisel.VagaryCry;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CChisel.ChiselCry + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CChisel.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CChisel.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
