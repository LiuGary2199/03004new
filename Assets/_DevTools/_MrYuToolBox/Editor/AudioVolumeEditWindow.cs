...
public class AudioVolumeEditWindow : EditorWindow
{
    ...
    Dictionary<string, AudioParis> UIAudioGroup;
    Dictionary<string, AudioParis> SceneAudioGroup;
    //序列化对象
    ...
    {
        UIAudioGroup = new Dictionary<string, AudioParis>();
        SceneAudioGroup = new Dictionary<string, AudioParis>();
        ...
        Dictionary<string, AudioParis> audioDic = new Dictionary<string, AudioParis>();
        if (FileInfo.Exists)
        {
        ...
        //UI
        foreach(object music in Enum.GetValues(typeof(GooseCity.UIMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            AudioParis model = new AudioParis();
        ...
        // Scene
        foreach (object music in Enum.GetValues(typeof(GooseCity.SceneMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            AudioParis model = new AudioParis();
    ...
    public Dictionary<string, AudioParis> LoadJson(string Filepath)
    {
        StreamReader reader = new StreamReader(Filepath);
        string jsonString = reader.ReadToEnd();
        Dictionary<string, AudioParis> audioGroup = JsonMapper.ToObject<Dictionary<string, AudioParis>>(jsonString);
        reader.Dispose();
        reader.Close();
        return audioGroup;
    }
    ...
    {
        Dictionary<string, AudioParis> dic = new Dictionary<string, AudioParis>();
        MergeDictionaries.Ounce(dic, UIAudioGroup);
        MergeDictionaries.Ounce(dic, SceneAudioGroup);
